...
// File Name: pay.php
//
// Purpose: Processing logic for customer transaction.
//
///////////////////////////////////////////////////
...
ERRORSTART;
	...
	<html>
	<head>
	<title>Newsagent: Billing Information</title>
	<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
	</head>
	...
	<div align="center">
	  ...
	  <div align="left"><br>
	    ...
	    <img src="../images/div2.jpg" width="400" height="17"><br>
	    <br>
	    <p>These are your customers' billing information. They will be charged daily when you delivery boy send the product.<br> 
	    When your customers make a payment, click Edit Balance to update.</p>	
STARTHEADER;
...
{				
			...
			$sqlQuery = "SELECT orderlist.id, orderlist.product_id, orderlist.customer_id, orderlist.quantity, product.name, product.type, product.price, customer.name, customer.address, customer.town, customer.state, customer.postcode, customer.telephone, customer.price FROM orderlist, product, customer, deliveryboy WHERE (orderlist.product_id = product.id) AND (orderlist.deliveryboy_id = deliveryboy.id) AND (product.newsagent_id = $newsagentId) AND (orderlist.customer_id = customer.id)" ;
						...
						while($sqlValues = mysql_fetch_array($sqlResult))
						{
							echo "<tr>";
								...
								//	echo "<td>$sqlValues[2]</td>\n";
									...
									echo "<td>RM $sqlValues[13]</td>\n";
									echo "<td> <a href=\"edit_balance.php?customerId=";
									echo "$sqlValues[2]\" ";
									echo ">Edit Balance</a>";		
								}
