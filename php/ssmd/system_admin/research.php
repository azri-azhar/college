    ...
    <td height="251" bgcolor="#DDE6EE"><table width="100%"  border="0" cellspacing="2" cellpadding="2">
      <tr>
        <td width="77%" height="354" valign="top" bgcolor="#FFFFFF"><table width="100%"  border="0" cellspacing="2" cellpadding="2">
          <tr>
              <td bgcolor="#EEEEDD"><strong>Research Contract Management Authorization </strong></td>
          </tr>
          </table>          
          <p>Manage username and password for Research Contract Management authorization.</p>
          <p>
          
		<form name="formLab" method="post" action="research.php" >
			<table>
				<tr>
					<td>Username</td>
			...
			if ( isset($_GET['deleteid']) )
			{
				...
				$sqlQuery = "DELETE FROM authorization WHERE class = 4 AND id = '$deleteid'";
			...
			if (isset( $_POST['txtNewUsername']) & (isset( $_POST['txtNewPassword'] ) ) )
			{
				...
				$sqlQuery = "INSERT INTO authorization(username, password, class) VALUES('$username', '$password', 4)";
				$sqlResult =  mysql_query($sqlQuery) or die("Can't Make query");
			...
			$sqlQuery = "SELECT id, username, password FROM authorization WHERE class = 4";
			$sqlResult =  mysql_query($sqlQuery) or die("Can't Make query");
			...
			while($sqlValues = mysql_fetch_array($sqlResult))
			{
				echo "<tr>";
					...
					for($column = 2; $column < $columnCount; $column++)
					{
						...
						echo "<td> <a href=\"research.php?deleteid=";
